...
class CC_Theme_Generator{
	...
	function header_logo(){
		global $cap;	
			...
			<?php if($cap->logo){ ?>
			<a href="<?php echo site_url() ?>" title="<?php _e( 'Home', 'cc' ) ?>"><img src="<?php echo $cap->logo?>" alt="<?php if(defined('BP_VERSION')){ bp_site_name(); } else { bloginfo('name'); } 
?>"></img></a>
			<?php } ?>
			</div>
		<?php else: ?>
	...
	function bp_menu(){
		...
		<?php else : ?>
			...
			<?php }?>
				...
				<li id="nav-community"<?php if ( bp_is_page( BP_ACTIVITY_SLUG ) || (bp_is_page( BP_MEMBERS_SLUG ) || bp_is_user()) || (bp_is_page( BP_GROUPS_SLUG ) || bp_is_group()) || bp_is_page( 
BP_FORUMS_SLUG ) || bp_is_page( BP_BLOGS_SLUG ) )  : ?> class="page_item current-menu-item"<?php endif; ?>>
					...
					<ul class="children">
						...
						<?php if ( bp_is_active( 'groups' ) ) : ?>
							...
							<?php if ( bp_is_active( 'forums' ) && ( function_exists( 'bp_forums_is_installed_correctly' ) && !(int) bp_get_option( 'bp-disable-forum-directory' ) ) && 
bp_forums_is_installed_correctly() ) : ?>
	...
	function slideshow_home(){
		...
		if(defined('BP_VERSION')){ 
			if($cap->enable_slideshow_home == 'all' || $cap->enable_slideshow_home == 'home' && is_home() || $cap->enable_slideshow_home  == 'home' && is_front_page() || $cap->enable_slideshow_home == 
'home' && bp_is_component_front_page( 'activity' ) || is_page() && isset($cc_page_options) && $cc_page_options['cc_page_slider_on'] == 1){
				echo cc_slidertop(); // located under wp/templatetags
			}
		} elseif($cap->enable_slideshow_home == 'all' || $cap->enable_slideshow_home == 'home' && is_home() || $cap->enable_slideshow_home == 'home' && is_front_page() || is_page() && isset($cc_page_options) 
&& $cc_page_options['cc_page_slider_on'] == 1){
			echo cc_slidertop(); // located under wp/templatetags
		}
	}
	...
	function footer_content(){ 
		...
		<div class="widgetarea cc-widget cc-widget-right">
  		...
  		<div class="clear"></div>
	  	<br />
		<?php if($cap->disable_credits_footer != true ){ ?>
			<br />
			<div class="credits"><?php printf( __( '%s is proudly powered by <a class="credits" href="http://wordpress.org">WordPress</a> and <a class="credits" href="http://buddypress.org">BuddyPress</a>. 
', 'cc' ), bloginfo('name') ); ?>
	...
	function custom_login() { 
		...
		<?php if($cap->bg_loginpage_backtoblog_fade_1 && $cap->bg_loginpage_backtoblog_fade_2){ ?>
			#backtoblog {
				 background: -moz-linear-gradient(center bottom , #<?php echo $cap->bg_loginpage_backtoblog_fade_1; ?>, #<?php echo $cap->bg_loginpage_backtoblog_fade_2; ?>) repeat scroll 0 0 
transparent;
			}
		<?php } ?>
		</style>
