...
jQuery(document).ready(function(){
	...
	jQuery("#d5framework-main-menu ul ul").css({display: "none"});

    jQuery('#d5framework-main-menu ul li').hover(

	    function() { jQuery(this).find('ul:first').slideDown(200).css('visibility', 'visible'); jQuery(this).addClass('selected'); },

		function() { jQuery(this).find('ul:first').slideUp(200); jQuery(this).removeClass('selected'); });
...
function slideShow() {
	...
	jQuery('#gallery .caption').css({opacity: 0.9});
...
function gallery() {
	...
	jQuery('#gallery .caption').animate({opacity: 0.7},100 ).animate({height: '60px'},500 );
	//Display the content
