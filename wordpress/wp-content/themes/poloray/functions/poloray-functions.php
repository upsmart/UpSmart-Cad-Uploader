...
add_action('init', 'poloray_register_custom_menu');

function poloray_register_custom_menu() {
 ...
 * @param type $ih 
 */
function poloray_get_thumbnail($iw, $ih) {
    $permalink = get_permalink();
    $thumb = get_post_thumbnail_id();
    $image = poloray_thumbnail_resize($thumb, '', $iw, $ih, true, 90);
 ...
 */
function poloray_get_image($width, $height) {
    $w = $width;
    $h = $height;
    global $post, $posts;
//This is required to set to Null
    ...
    $img_path = poloray_image_resize($img_source, $w, $h);
...
function poloray_childtheme_analytics() {
    $output = poloray_get_option('poloray_analytics');
    if ($output <> "")
        echo stripslashes($output) ;
}

add_action('wp_head', 'poloray_childtheme_analytics');
/* ----------------------------------------------------------------------------------- */
/* Custom CSS Styles */
/* ----------------------------------------------------------------------------------- */
